...
class PolicyHit:
    ...
    meta: Dict[str, Any]  # contains source, source_title, section, is_catalog, weight, doc_id, etc.
...
class PolicyIndex:
    ...
    @property
    def size(self) -> int:
        """Number of indexed chunks (for UI display)."""
        return int(self.matrix.shape[0]) if self.matrix is not None else 0
