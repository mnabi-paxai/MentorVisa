...
def status_dot(color: str) -> str:
    return f"<span style='display:inline-block;width:8px;height:8px;border-radius:50%;background:{color};margin-right:6px;vertical-align:middle'></span>"
...
def agent_overview_cards():
    ...
    for i, m in enumerate(metrics):
        with cols[i % len(cols)]:
            color = {"active": "#16a34a", "in-progress": "#f59e0b", "idle": "#6b7280", "flagged": "#ef4444"}.get(m.get("status", "idle"), "#6b7280")
            st.markdown(
                f"""
...
FALLBACKS = {
    "compliance_checker.png": "/mnt/data/Simple-first-AIAssistant-Hike.png",
    "form_filer.png": "/mnt/data/whatsapp_basic.png",
    "audit_prep.png": "/mnt/data/Whatspp_try1.png",
}

def resolve_img(name: str) -> Path | None:
    """
    Prefer local asset ./assets/agents/<name>.
    If not present, try the provided fallback absolute path.
...
def safe_image(path_or_none: Path | None, alt: str, *, height: int | None = None):
    ...
    if path_or_none and path_or_none.exists():
        st.image(str(path_or_none), width="stretch")
        return
    # placeholder card
...
with tab_ops:
    ...
    for i, card in enumerate(AGENT_CARDS):
        with cols[i]:
            color = {"active": "#16a34a", "in-progress": "#f59e0b", "idle": "#6b7280", "flagged": "#ef4444"}.get(card["status"], "#6b7280")
            st.markdown(
                f"""
                <div style="border:1px solid #eee;border-radius:16px;padding:10px;margin-bottom:10px;">
                    <div style="font-size:13px;color:#6b7280;">{status_dot(color)}{card['role']}</div>
                    <div style="font-size:16px;font-weight:600;margin:2px 0 8px 0;color:#111827;">{card['name']}</div>
                </div>
                """,
                unsafe_allow_html=True,
            )
            ...
            st.markdown(
                ...
                <div style="display:flex;flex-direction:column;
                    ...
                    <div>
                        <div style="font-size:13px;color:#6b7280;">{status_dot(color)}{card['role']}</div>
                        <div style="font-size:16px;font-weight:600;margin:2px 0 8px 0;color:#111827;">{card['name']}</div>
                        <img src="file://{card['image']}" style="width:100%;height:160px;object-fit:cover;border-radius:8px;"/>
                    </div>
                    <button style="margin-top:12px;padding:8px 0;border:1px solid #ddd;
                                border-radius:8px;background:#fff;width:100%;">Open</button>
                </div>
                """,
                unsafe_allow_html=True,
            )
    ...
    if st.session_state.get("open_cc"):
        with st.expander("Compliance Checker — quick actions", expanded=True):
            ...
            run_cc = st.button("Run Scan", width="stretch")
            cc_fn = get_agent_fn("run_compliance_scan")
            if run_cc:
                with st.spinner("Scanning…"):
                    try:
                        findings = cc_fn(scope=scope, review_required=review_mode) if callable(cc_fn) else [
                            {"doc": "Vendor_A_Master_Agreement.pdf", "issue": "Missing DP addendum", "severity": "High"}
                        ]
    ...
    if st.session_state.get("open_ff"):
        with st.expander("Form Filer — quick actions", expanded=True):
            ...
            run_ff = st.button("Submit", width="stretch")
            ff_fn = get_agent_fn("file_form")
            if run_ff:
                with st.spinner("Submitting…"):
                    try:
                        result = ff_fn(form_type=form_type, payload_file=None, dry_run=dry_run) if callable(ff_fn) else {
                            "status": "ok", "submission_id": "SUBM-2025-0912-XYZ"
                        }
    ...
    if st.session_state.get("open_ap"):
        with st.expander("Audit Prep — quick actions", expanded=True):
            ...
            run_ap = st.button("Generate Pack", width="stretch")
            ap_fn = get_agent_fn("generate_audit_pack")
            if run_ap:
                with st.spinner("Packaging…"):
                    try:
                        pack = ap_fn(start=start, end=end, include=["Access Logs", "Approvals"]) if callable(ap_fn) else {
                            "status": "ok", "file_name": f"audit_pack_{start}_{end}.zip", "size_mb": 12.4
                        }
...
with tab_eng:
    ...
    with st.expander("Stand-up Summarizer", expanded=False):
        if st.button("Summarize last 24h", width="stretch"):
    ...
    with st.expander("Prep a 1:1", expanded=False):
        ...
        if st.button("Prepare 1:1 agenda", width="stretch"):
